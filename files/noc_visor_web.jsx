...
function useNotificaciones() {
  ...
  const capPlugin = useRef(null);
  const esNativo  = useRef(false);

  useEffect(() => {
    // Detectar si estamos en Capacitor (app nativa iOS/Android)
    const isNative = typeof window !== "undefined" &&
      window.Capacitor?.isNativePlatform?.();
    esNativo.current = isNative;

    if (isNative) {
      // Cargar plugin de Capacitor dinámicamente
      import("@capacitor/push-notifications").then(({ PushNotifications }) => {
        capPlugin.current = PushNotifications;
        PushNotifications.checkPermissions().then(res => {
          setPermiso(res.receive === "granted" ? "granted" : "default");
        });
      }).catch(() => {});
    } else {
  ...
  async function pedirPermiso() {
    if (esNativo.current && capPlugin.current) {
      // Nativo: pedir permiso via Capacitor
      const res = await capPlugin.current.requestPermissions();
      if (res.receive === "granted") {
        await capPlugin.current.register();
        setPermiso("granted");
      }
    } else if (typeof Notification !== "undefined") {
      // Web: pedir permiso del navegador
  ...
  function notificar(titulo, cuerpo) {
    ...
    if (esNativo.current) {
      // Nativo iOS/Android: LocalNotifications
  ...
  function notificarCaida(rutas) {
    notificar(
      "ALERTA NOC - CAIDA DETECTADA",
      `${rutas.length} caida${rutas.length > 1 ? "s" : ""} activa${rutas.length > 1 ? "s" : ""}:\n${rutas.join("\n")}`
    );
  }
  ...
  return { permiso, pedirPermiso, notificarCaida, notificarRecuperado };
}
...
function TabMonitoreo({ esAdmin }) {
  ...
  const { permiso, pedirPermiso, notificarCaida, notificarRecuperado } = useNotificaciones();
  ...
  return (
    <div className="fade-in">
      ...
      <div style={{ display: "flex", gap: 12, marginBottom: 20, flexWrap: "wrap" }}>
        ...
        <div style={{ ...S.statCard(permiso === "granted" ? "var(--success)" : "var(--warn)"),
          ...
          {permiso === "granted" ? (
            ...
            <div style={{ fontSize: 11, color: "var(--danger)", textAlign: "center" }}>
              🚫 Bloqueadas<br/>
              <span style={{ color: "var(--sub)", fontSize: 10 }}>Actívalas en config. del navegador</span>
            </div>
          ) : (
